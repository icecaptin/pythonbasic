...
class Numeric(Problem):
    ...
    def setVariables(self, parameters):
        ...
        self._expression = infile.readline()  # 수식 읽어오기
        varNames = []
        low = []
        up = []
    ...
    def report(self):
        ...
        print("Best Minimum value: {0:,.3f}".format(self._bestMinimum))  # 최적해의 평가 값 출력
        Problem.report(self)  # 부모 클래스의 report 메서드 호출

    def coordinate(self):
        c = [round(value, 3) for value in self._bestSolution]  # 최적해의 좌표값을 소수점 3자리로 반올림
        return tuple(c)
...
class Tsp(Problem):
    ...
    def report(self):
        ...
        print("Perfect order of visits:")
        self.tenPerRow()  # 최적 순서 출력
        print("Best Minimum tour cost: {0:,}".format(round(self._bestMinimum)))  # 최적 순서의 평가 값 출력
        Problem.report(self)  # 부모 클래스의 report 메서드 호출
